...
public class ParallelFWTopicModel implements Serializable {

	public static final int UNASSIGNED_TOPIC = -1;

	public static Logger logger = MalletLogger.getLogger(ParallelFWTopicModel.class.getName());
	...
	public ParallelFWTopicModel (int numberOfTopics) {
		this (numberOfTopics, numberOfTopics, DEFAULT_BETA);
	}
	
	public ParallelFWTopicModel (int numberOfTopics, double alphaSum, double beta) {
	...
	public ParallelFWTopicModel (LabelAlphabet topicAlphabet, double alphaSum, double beta) {
	...
	public void buildInitialWordTopicCounts () {
		
		wordTopicCounts = new int[numTypes][];
		tokensPerTopic = new int[numTopics];
                wordTopicIndex=new int[numTypes][];
		...
		for (int type = 0; type < numTypes; type++) {
			wordTopicCounts[type] = new int[ numTopics ];
                        wordTopicIndex[type] = new int[ numTopics ];
		}
		...
		for (TopicAssignment document : data) {
			...
			for (int position = 0; position < tokens.size(); position++) {
				...
				int type = tokens.getIndexAtPosition(position);
				wordTopicCounts[ type ][topic]++;
                                wordTopicIndex[type][topic]=topic;
			}
		}
                // Call Quick sort to sort the index array based on values
                // wordTopicCounts 
                QuickSort qt = new QuickSort();
                for(int type=0;type<numTypes;type++)
                    qt.sort(wordTopicCounts[type], 0, wordTopicCounts[type].length-1, wordTopicIndex[type]);
                
	}
	

	public void sumWordTopicCounts (WorkerRunnableFW[] runnables) {
		...
		for (int thread = 0; thread < numThreads; thread++) {
			...
			int[][] sourceWordTopicCounts = 
				runnables[thread].getWordTopicCounts();
			
			for (int type = 0; type < numTypes; type++) {
                            
                            for(int topic =0;topic<numTopics;topic++)
                                wordTopicCounts[type][topic] += sourceWordTopicCounts[type][topic];
			}
		}
	}
	...
	public void optimizeAlpha(WorkerRunnableFW[] runnables) {
	...
	public void temperAlpha(WorkerRunnableFW[] runnables) {
	...
	public void optimizeBeta(WorkerRunnableFW[] runnables) {
		// The histogram starts at count 0, so if all of the
		//  tokens of the most frequent type were assigned to one topic,
		//  we would need to store a maxTypeCount + 1 count.
		int[] countHistogram = new int[numTypes + 1];
		...
		int index;
		for (int type = 0; type < numTypes; type++) {
			int[] counts = wordTopicCounts[type];
                        int[] indexes= wordTopicIndex[type];
			index = 0;
			while (index < indexes.length &&
				   counts[indexes[index]] > 0) {
				...
				index++;
                                ...
                                {
                                    System.out.println("countHistogram length "+countHistogram.length+" index "+ind+" count[ind] "+counts[ind]);
                                }
			}
		}
	...
	public void estimate () throws IOException {
		...
		WorkerRunnableFW[] runnables = new WorkerRunnableFW[numThreads];
		...
		if (numThreads > 1) {
			...
			for (int thread = 0; thread < numThreads; thread++) {
				...
				int[][] runnableCounts = new int[numTypes][];
                                int[][] runnableIndex = new int[numTypes][];
				for (int type = 0; type < numTypes; type++) {
					int[] counts = new int[wordTopicCounts[type].length];
					System.arraycopy(wordTopicCounts[type], 0, counts, 0, counts.length);
					runnableCounts[type] = counts;
                                        int[] ind = new int[wordTopicIndex[type].length];
					System.arraycopy(wordTopicIndex[type], 0, ind, 0, ind.length);
					runnableIndex[type] = ind;
				}
				...
				runnables[thread] = new WorkerRunnableFW(numTopics,
													   alpha, alphaSum, beta,
													   random, data,
													   runnableCounts,runnableIndex, runnableTotals,
			...
			runnables[0] = new WorkerRunnableFW(numTopics,
											  alpha, alphaSum, beta,
											  random, data,
											  wordTopicCounts,wordTopicIndex, tokensPerTopic,
											  offset, docsPerThread);
		...
		for (int iteration = 1; iteration <= numIterations; iteration++) {
			...
			if (numThreads > 1) {
				...
				for (int thread = 0; thread < numThreads; thread++) {
					...
					int[][] runnableCounts = runnables[thread].getWordTopicCounts();
					for (int type = 0; type < numTypes; type++) {
						int[] targetCounts = runnableCounts[type];
						int[] sourceCounts = wordTopicCounts[type];
	...
	public ArrayList<TreeSet<IDSorter>> getSortedWords () {
		...
		for (int type = 0; type < numTypes; type++) {
			...
			int[] topicCounts = wordTopicCounts[type];
                        int[] topicIndex = wordTopicIndex[type];
			int index = 0;
                        while(index< topicIndex.length && topicCounts[topicIndex[index]]>0)
                        {
                            int topic = topicIndex[index];
		...
		for (int topic = 0; topic < numTopics; topic++) {
			for (int type = 0; type < numTypes; type++) {
				...
				int index = 0;
                                ...
                                while(index<topicIndex.length && topicCounts[topicIndex[index]]>0)
                                {
                                    int currentTopic = topicIndex[index];
                                    if (currentTopic == topic) {
						weight += topicCounts[topic] ;
						break;
					}
	...
	public double modelLogLikelihood() {
		...
		int[] topicCounts = new int[numTopics];
                int [] topicIndex = new int [numTopics];
		...
		for (int type=0; type < numTypes; type++) {
			...
			topicCounts = wordTopicCounts[type];
                        topicIndex = wordTopicIndex[type];
			int index = 0;
	...
	public TopicInferencer getInferencer() {
		return new TopicInferencer(wordTopicCounts, tokensPerTopic,
	...
	public MarginalProbEstimator getProbEstimator() {
		return new MarginalProbEstimator(numTopics, alpha, alphaSum, beta,
										 wordTopicCounts, tokensPerTopic);
	}
	...
	private void writeObject (ObjectOutputStream out) throws IOException {
		...
		out.writeDouble(betaSum);
                ...
                out.writeObject(wordTopicIndex);
	...
	private void readObject (ObjectInputStream in) throws IOException, ClassNotFoundException {
		...
		wordTopicCounts = (int[][]) in.readObject();
                wordTopicIndex = (int[][]) in.readObject();
	...
	public static ParallelFWTopicModel read (File f) throws Exception {

		ParallelFWTopicModel topicModel = null;

		ObjectInputStream ois = new ObjectInputStream (new FileInputStream(f));
		topicModel = (ParallelFWTopicModel) ois.readObject();
		ois.close();
	...
	public static void main (String[] args) {
		...
		try {
			...
			ParallelFWTopicModel lda = new ParallelFWTopicModel (numTopics, 50.0, 0.01);
